...
class ControlCenterStorageService {
  ...
  Future<ControlCenterConfig> loadConfig() async {
    ...
    final local = localRaw == null || localRaw.isEmpty
        ? defaults
        : ControlCenterConfig.decode(localRaw);

    // Read old Mezo keys from Settings.System when the native bridge is available.
    // If the bridge is unavailable, these calls safely return local/default values.
    final squareTiles = await AndroidIntentBridge.readBool(
      ControlCenterConfig.squareMezoTilesKey,
      defaultValue:
          prefs.getBool(ControlCenterConfig.squareMezoTilesKey) ??
          local.squareMezoTiles,
    );

    final style = await AndroidIntentBridge.readInt(
      ControlCenterConfig.controlCenterStyleKey,
      defaultValue:
          prefs.getInt(ControlCenterConfig.controlCenterStyleKey) ??
          local.controlCenterStyle,
    );

    final extraTiles = await AndroidIntentBridge.readString(
      ControlCenterConfig.extraTwoMezoTilesKey,
      defaultValue:
          prefs.getString(ControlCenterConfig.extraTwoMezoTilesKey) ??
          local.extraTwoAsBridgeString,
    );

    final blur = await AndroidIntentBridge.readInt(
      ControlCenterConfig.ccBlurRatioKey,
      defaultValue:
          prefs.getInt(ControlCenterConfig.ccBlurRatioKey) ??
          local.ccBlurRatio,
    );
  ...
  Future<void> saveConfig(ControlCenterConfig config) async {
    ...
    await AndroidIntentBridge.writeBool(
      ControlCenterConfig.squareMezoTilesKey,
      config.squareMezoTiles,
    );

    await AndroidIntentBridge.writeInt(
      ControlCenterConfig.controlCenterStyleKey,
      config.controlCenterStyle,
    );

    await AndroidIntentBridge.writeString(
      ControlCenterConfig.extraTwoMezoTilesKey,
      config.extraTwoAsBridgeString,
    );

    await AndroidIntentBridge.writeInt(
      ControlCenterConfig.ccBlurRatioKey,
      config.ccBlurRatio.clamp(0, 100).toInt(),
    );
  }

  Future<void> _saveLocalMirrors(
